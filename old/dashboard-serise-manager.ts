...
export class SeriseManager{
    ...
    public static get getInstance(){   // can create one and only one instance " singleton "
    ...
    public prepareSerise(Queries: dash.Query[], Data: any[], removeNull: boolean = false): any[] {
        ...
        return Data = GroupingManager.getInstance.groupByOperations(Q,Data,removeNull); //GroupBy_v1(Q, Data, removeNull); 
    }
    public prepareSerise_withFields(Queries: dash.Query[], Data: any[], removeNull: boolean = false, sortGroups: boolean = true): PreparedDataGroups {
        let Q: dash.GroupOperation[] = this.convertQueryToGroupOperation(Queries);
        return GroupingManager.getInstance.groupWithFields(Q, Data, dash.QueryTypeEnum.Serise, removeNull, sortGroups);//ConstructGroups_WithFields(Q, Data, dash.QueryTypeEnum.Serise, removeNull, sortGroups);
    }
